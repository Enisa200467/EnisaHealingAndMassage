import type { BookingFormData, Treatment, TimeSlot, BookingConfirmation } from '../types/booking.types'
...
export const useBooking = () => {
   ...
   */
  const submitBooking = async (data: BookingFormData): Promise<{ success: boolean; confirmation?: BookingConfirmation; error?: string }> => {
    try {
      ...
      const confirmation: BookingConfirmation = {
        ...
        confirmationNumber: `ENM${Math.random().toString(36).substring(2, 8).toUpperCase()}`
      }
